...
// Gets header titles. Used to help ensure valid input for choice in buildProductTotalsList 
...
// Helper - Gets the column index for the headers to be able to target which column you're grouping by. I kept this generic vs right away targetting product_name.
...
//Sums the total of the user input choice. (ie if they chose revenue, it will total revenue by product_name )

function buildProductTotalsList (choice) {
	IF choice == product_name OR choice == product_id
		DISPLAY "Invalid. Cannot rank top/bottom 5 product by product"
		RETURN
...
//Displays the ranked values. ****Sort may need a function (which may be part of part 1??)
